...
def process_patient(
    ...
    for i, dicom in enumerate(tqdm(dicoms)):
        ...
        # mask slice
        masked_img, binary_img = lung_mask(windowed_img) # mask is masked image, binary is mask
...
def save_to_disk(patient_volumes=None,
    ...
    if SAVE_TRAPEZOID_VOLUMES:
        ...
        filestring = os.path.splitext(csv_path)[0] + '_mod' + os.path.splitext(csv_path)[1]
...
def main():
    ...
    SAVE_TRAPEZOID_VOLUMES = True   # Save the trapezoid estimated volume for each patient
