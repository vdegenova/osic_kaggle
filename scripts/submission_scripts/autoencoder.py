...
def create_jesse_autoencoder(img_px_size=64, slice_count=8, lr=1e-4):
    """
    this model assumes (64, 64, 8) is the dimensionality of the 3D input
    """
    tf.keras.backend.set_image_data_format("channels_last")
    ...
    autoencoder = keras.Sequential([encoder, decoder], name="sequential_autoencoder")
...
def get_training__and_validation_data_and_patients(
    filename="./data/processed_data/allthedata-condensed-with_ids-32-32-8.npy",
):
...
def get_test_data_and_patients(filename):
    all_data = np.load(filename, allow_pickle=True)
    np.random.shuffle(all_data)
    ...
    imgs_x = test_data[:, 0]
    imgs_x = np.array([x[:] for x in imgs_x])
    test_x = np.moveaxis(imgs_x, 1, -1)
    patient_ids_x = test_data[:, 1]
    normalized_testing_images = normalize(test_x)

    return normalized_testing_images, patient_ids_x
...
def train_model(model, training_data, val_data, suffix=None, n_epochs=10):
    ...
    tensorboard_callback = TensorBoard(
        log_dir=log_dir, histogram_freq=3
...
def kt_model_builder(
    ...
    hp_lr = hp.Float(
        "learning_rate", min_value=1e-5, max_value=1e-2, sampling="LOG", default=1e-3
    )
