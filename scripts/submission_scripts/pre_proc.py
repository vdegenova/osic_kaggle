...
def make_lungmask(img, display=False):
    ...
    labels = (
        labels + 1
    )  # add 1 to every element so that the background is no longer encoded as 0
    ...
    for prop in regions:
        ...
        if (
            B[2] - B[0] < row_size * 0.90
            and B[3] - B[1] < col_size * 0.90
            and B[0] > row_size * 0.075
            and B[2] < row_size * 0.925
            and B[1] > col_size * 0.075
            and B[3] < col_size * 925
            and prop.area / n_px * 100 > 0.1
        ):
...
def make_lungmask_v2(img):
    ...
    thresh, thresh_img = threshold(
        img.astype("uint8"), np.min(img), np.max(img), THRESH_OTSU
    )
...
def process_data(
    ...
    for each_slice in tqdm(slices):
        ...
        if mask_var > 0.04 and n_labels < 10:
    ...
    for each_slice in masked_slices:
        ...
        resized_masked_slices.append(
            resize(
                np.array(each_slice[xmin:xmax, ymin:ymax]), (img_px_size, img_px_size)
            )
        )
    ...
    chunked_slices = np.zeros(
        shape=(hm_slices, img_px_size, img_px_size)
    )  # (8, 64, 64)
    for i in range(chunked_slices.shape[1]):
        chunked_slices[:, i, :] = resize(
            resized_masked_slices[:, i, :], (img_px_size, hm_slices)
        )
    ...
    if verbose:
        print(
            f"Patient {patient}; max: {chunked_slices.max()}, min: {chunked_slices.min()}"
        )
...
def read_in_data(
    ...
    verbose=False,
    training=True,
    csv_file="/kaggle/input/osic-pulmonary-fibrosis-progression/train.csv",
):
    """
    Function to ___
    INPUTS:
    ...
    train_df = pd.read_csv(csv_file)  # list of training patient tabular data
    ...
    for num, patient in enumerate(patients):
        ...
        try:
            img_data, patient_history = process_data(
                patient,
                patient_history_df,
                img_px_size=IMG_PX_SIZE,
...
def save_to_disk(
    data, img_px_size=32, slice_count=8, test_data=False, working_dir="/kaggle/working/"
):
    now = datetime.datetime.now().isoformat(timespec="minutes")
    test_str = ""
    if test_data:
        test_str += "test_data-"
    filestring = f"{working_dir}{test_str}{data.shape[0]}-images-with_ids-{img_px_size}-{img_px_size}-{slice_count}-{now}.npy"
